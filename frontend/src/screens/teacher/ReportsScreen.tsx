...
export function TeacherReportsScreen() {
  ...
  const handleSave = async () => {
    if (!studentUuid || !form.title.trim() || !form.content_markdown.trim()) {
      setError('Student, title, and content are required.');
      return;
    }
    if (!!form.period_start !== !!form.period_end) {
      setError('Period start and end must be provided together.');
      return;
    }
    setSaving(true);
    setError('');
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 16 }}>
        <div className="card">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            ...
            <input className="input-field" placeholder="Report title" value={form.title} onChange={e => setForm(prev => ({ ...prev, title: e.target.value }))} />
            ...
            <input className="input-field" placeholder="Original language" value={form.original_language} onChange={e => setForm(prev => ({ ...prev, original_language: e.target.value }))} />
          </div>
          <textarea className="input-field" rows={12} placeholder="Report markdown" value={form.content_markdown} onChange={e => setForm(prev => ({ ...prev, content_markdown: e.target.value }))} style={{ resize: 'vertical', fontFamily: 'var(--font-body)' }} />
          {error && <div style={{ marginTop: 10, fontSize: 12, color: 'var(--warn)' }}>{error}</div>}
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <button className="btn-primary" onClick={() => void handleSave()} disabled={saving}>
              {saving ? 'Saving…' : editingUuid ? 'Update Report' : 'Create Report'}
            </button>
            {editingUuid && (
        ...
        <div className="card">
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--tx)', marginBottom: 16 }}>Session Reports</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {reports.map(report => (
              <div key={report.uuid} className="card-sm">
                ...
                <div style={{ fontSize: 12, color: 'var(--tx3)', marginBottom: 6 }}>
                  {report.report_type} · {report.subject?.name ?? 'All subjects'} · {report.author.display_name}
                </div>
            ...
            {reports.length === 0 && (
              <div style={{ fontSize: 13, color: 'var(--tx3)' }}>
                Reports created or updated in this session will appear here for quick editing.
              </div>
            )}
          </div>
        </div>
