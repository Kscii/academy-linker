...
import { mockTeacherStudents, mockTeacherStudentThreads, SUBJECT_COLORS } from '@/lib/mock-data';
import { teacher as teacherApi } from '@/lib/api';
import type { TeacherStudentItem } from '@/types/api';
...
export function TeacherMessagesScreen() {
  const { markThreadRead, threadUnreadCounts, conversations, sendMessage, language } = useApp();
  ...
  const activeThreadUuid = mockTeacherStudentThreads[activeStudentUuid] ?? `teacher-thread-${activeStudentUuid}`;
  const messages: DirectMessage[] = conversations[activeThreadUuid] ?? [];

  const sendReply = async (text: string) => {
    if (!text.trim()) return;
    await sendMessage(activeThreadUuid, 'teacher', text, language);
    setReply('');
        ...
        <div className="conversation-list">
          ...
          {students.map(item => (
            ...
            >
              ...
              {(threadUnreadCounts[item.student.uuid] ?? item.unread_messages) > 0 && (
                <div style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--a1)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, flexShrink: 0 }}>
                  {threadUnreadCounts[item.student.uuid] ?? item.unread_messages}
                </div>
              )}
            </div>
          ))}
        ...
        <div className="message-thread">
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--bd)', display: 'flex', alignItems: 'center', gap: 12, background: 'var(--card)' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--tx3)' }}>Parent: Li Wei · {activeStudent.student.class_name}</div>
            </div>
          ...
          <div className="thread-messages">
            {messages.length === 0 && (
              <div style={{ textAlign: 'center', color: 'var(--tx3)', fontSize: 13, padding: '40px 0' }}>No messages yet.</div>
            )}
            {messages.map(msg => {
              const isTeacher = msg.sender === 'teacher';
              const displayName = isTeacher ? 'Ms. Thompson' : 'Li Wei';
              return (
                <div key={msg.uuid} style={{ display: 'flex', gap: 10, flexDirection: isTeacher ? 'row-reverse' : 'row' }}>
                  ...
                  <div style={{ maxWidth: '70%' }}>
                    <div style={{ fontSize: 11, color: 'var(--tx3)', marginBottom: 4, textAlign: isTeacher ? 'right' : 'left' }}>
                      {displayName} · {timeAgo(msg.sent_at)}
                    </div>
          ...
          <div className="thread-input-area">
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
              ...
              />
              <button className="btn-primary" style={{ width: 'auto', padding: '8px 18px', flexShrink: 0, alignSelf: 'flex-end' }} onClick={() => sendReply(reply)} disabled={!reply.trim()}>
                Send
              </button>
            </div>
          </div>
